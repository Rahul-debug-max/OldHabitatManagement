...
(function ($) {
    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            var emptydiv = $("<div class='col-xs-12 col-sm-12 col-md-12 col-lg-12' id='" + this.id + "_EmptyDv' ><div class='form-group' style='text-align:center;'><strong id='" + this.id + "_EmptyDvText'></strong></div></div>").insertAfter(gv);
            ...
            updatepager = function (rn, dnd) {
                if ($("#" + ts.id + "_EmptyDv").length > 0) {
                    if ($("#" + ts.id + "_EmptyDvText").length > 0) {
                        $("#" + ts.id + "_EmptyDvText").html(ts.p.emptyrecords);
                    }
                    $("#" + ts.id + "_EmptyDv").hide();
                }
                ...
                if (pgboxes) {
                    ...
                    if (ts.p.pginput === true) {
                        $('.ui-pg-input', pgboxes).text(ts.p.page);
            ...
            setPager = function (pgid, tp) {
                ...
                $("#" + $.jgrid.jqID(pgid))
                //.append("<div id='" + pgcnt + "' class='ui-pager-control' role='group' style='padding-right:15px;'><div style='padding-right:15px;'><div class='pull-right' style='width:174px;' id='" + cent + "'><label class='control-label' style='float:left; line-height:34px; padding:0 20px; color:#808080; font-weight:normal;'>View</label><select class='form-control' style='width:100px;'><option>20</option><option>50</option><option>100</option></select></div><ul class='pagination pull-right' style='margin:0 0 15px 0;'><li><a href='#'>«</a></li><li class='active'><a href='#'>1 <span class='sr-only'>(current)</span></a></li><li><a href='#'>2</a></li><li><a href='#'>...</a></li><li><a href='#'>4</a></li><li><a href='#'>5</a></li><li><a href='#'>»</a></li></ul></div></div>")
                    .append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><div id='" + cent + "' ></div></div>")

                if (ts.p.rowList.length > 0) {
                    str = "<div class='pull-right form-inline' style='padding-right:20px'>";

                    //str += "<select class='ui-pg-selbox form-control' style='width:100px;' role='listbox'>";
                    //for (i = 0; i < ts.p.rowList.length; i++) {
                    //    if (ts.p.rowList[i] == 'ALL') {
                    //        str += "<option role=\"option\" value=\"2147483647\"" + ((ts.p.rowNum === ts.p.rowList[i]) ? " selected=\"selected\"" : "") + ">" + ts.p.rowList[i] + "</option>";
                    //    }
                    //    else {
                    ...
                    //str += "</select>";
                    str += "<label class='control-label jqgrid-pg-label pull-left'>" + ts.p.pagerViewLabel + "</label>";
                    str += "<div class='form-group pull-left' style='width:80px;'><div class='input-group jqgrid-pg-size'>";                    
                    str += "<input type='text' class='form-control' id='" + pgcnt + "_selinputbox' value=" + ts.p.rowNum + " style='width:50px'>";
                    str += "<div class='input-group-addon inputSimpleBtn'>";
                    str += "<button type='button' class='btn btn-default dropdown-toggle inputSimpleBtnBorder' data-toggle='dropdown' aria-expanded='false'>";
                    str += "<span class='caret'></span>";
                    str += "<span class='sr-only'>Toggle Dropdown</span>";
                    str += "</button>";
                    str += "<ul class='dropdown-menu' role='menu' id='" + pgcnt + "_dropdown' >";
                    for (i = 0; i < ts.p.rowList.length; i++) {
                        str += "<li><a href='#' pagesize='" + ts.p.rowList[i] + "' >" + ts.p.rowList[i] + "</a></li>";
                    }
                    str += "</ul>";
                    str += "</div></div></div>";
                    str += "</div>";
                ...
                if (ts.p.pgbuttons === true) {
                    ...
                    pgl += "<ul class='pagination pull-right' style='margin:0 0 15px 0;'>";
                    pgl += "<li><a id='" + po[0] + "' ><span class='glyphicon glyphicon-step-backward'></span></a></li>";
                    pgl += "<li><a id='" + po[1] + "' ><span class='glyphicon glyphicon-backward'></span></a></li>";
                    //pgl += "<li class='active'>" + $.jgrid.format(ts.p.pgtext || "", "<a href='#'><label  class='ui-pg-input'></label><span class='sr-only'>(current)</span></a>", "<span id='sp_1_" + $.jgrid.jqID(pgid) + "' style='display:none'></span>") + "</li>";
                    pgl += pginp !== "" ? sep + pginp + sep : "";
                    pgl += "<li><a id='" + po[2] + "' ><span class='glyphicon glyphicon-forward'></span></a></li>";
                    pgl += "<li><a id='" + po[3] + "' ><span class='glyphicon glyphicon-step-forward'></span></a></li>";
                    pgl += "</ul>";
                ...
                $("#" + pgcnt + " .jqgrid-pg-size ul li a").bind('click', function () {
                    if (!clearVals('records')) { return false; }
                    var rowNumber = 10;
                    var strMenuValue = $(this).attr("pagesize");                   
                    if(strMenuValue.toUpperCase() == ts.p.pagerViewAllLabel.toUpperCase())
                    {
                        rowNumber = 2147483647
                    }
                    else {
                        rowNumber = parseInt(strMenuValue);
                    }
                    ts.p.page = Math.round(ts.p.rowNum * (ts.p.page - 1) / rowNumber - 0.5) + 1;

                    ts.p.rowNum = rowNumber;
                    if (ts.p.pager) { $('.ui-pg-selbox', ts.p.pager).val(rowNumber); }
                    if (ts.p.toppager) { $('.ui-pg-selbox', ts.p.toppager).val(rowNumber); }
                    $("#" + pgcnt + "_selinputbox").val(strMenuValue);
                    populate();
                    $("#" + pgcnt + "_dropdown").dropdown("toggle");
                    return false;

                });

                $("#" + pgcnt + " #" + pgcnt + "_selinputbox").bind('blur', function () {
                    if (!clearVals('records')) { return false; }                   
                    var pgSizeVal = $.trim($(this).val()).toUpperCase();
                    var rowNumber = 10;
                    if (pgSizeVal == "ALL") {
                        rowNumber = 1000;
                        $(this).val(pgSizeVal);
                        ...
                        if (pgSizeVal == "" || !$.isNumeric(pgSizeVal)) {
                            rowNumber = 10;
                        }
                        else if (Math.ceil(pgSizeVal) != pgSizeVal) {
                            rowNumber = Math.ceil(pgSizeVal);
                        }
                        else {
                            rowNumber = pgSizeVal;
